...
This safety plan can also be viewed online [at this link](`r knitr::asis_output(link_repo)`).  A summary of sites to be potentially assessed is included as Table \@ref(tab:tab-sites),  details of fish species potentially encountered is presented in  Table \@ref(tab:tab-fish) and an overview map of displaying potential sample locations is included as Figure 1. A kml file of the sites is included as an attachment to the application and can also be downloaded [from here at this link](`r knitr::asis_output(link_kml)`).  
...
table_sites %>% 
  ...
  select(id = aggregated_crossings_id, Stream = gnis_name, watershed_code_50k, watershed_code_20k, lat, long, sp_upstr = observedspp_upstr, source) %>% 
